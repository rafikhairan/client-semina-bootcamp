...
function Categories() {
	...
	const handleDelete = (id) => {
		Swal.fire({
			...
			if (result.isConfirmed) {
				...
				dispatch(setNotif(true, "success", `Berhasil hapus kategori ${res.data.data.name}`));
				dispatch(fetchCategories());
			}
		});
	};
	...
	return (
		<Container className="mt-3">
			...
			<Table status={categories.status} thead={["Nama", "Aksi"]} data={categories.data} tbody={["name"]} editUrl={access.edit ? `/categories/edit` : null} deleteAction={access.hapus ? (id) => handleDelete(id) : null} withoutPagination />
		</Container>
	);
}

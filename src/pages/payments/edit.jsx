...
function PaymentsEdit() {
	...
	const handleSubmit = async () => {
		...
		if (res.data.data) {
			dispatch(setNotif(true, "success", `berhasil ubah payments ${res.data.data.type}`));
			navigate("/payments");
			setIsLoading(false);
		} else {
			setIsLoading(false);
	...
	return (
		<Container>
			<SBreadCrumb textSecond={"Payments"} urlSecond={"/payments"} textThird="Edit" />
			{alert.status && <SAlert type={alert.type} message={alert.message} />}
			<Form form={form} isLoading={isLoading} handleChange={handleChange} handleSubmit={handleSubmit} edit />
		</Container>
	);
}
